...
def add_members(team, cpfs)
  cpfs.each do |cpf|
    ...
    TeamMembership.find_or_create_by!(team: team, user: user) if user
  end
end

# Carregadores
team = Team.find_or_create_by!(name: "Equipe Carregadores", sector: find_sector("Carregadores", event))
...
# Limpeza
team = Team.find_or_create_by!(name: "Equipe Limpeza", sector: find_sector("Limpeza", event))
...
# Guarda Volumes
team = Team.find_or_create_by!(name: "Equipe Guarda Volumes", sector: find_sector("Guarda Volumes", event))
add_members(team, %w[121.186.756-22 130.760.326-23 101.587.516-55 141.128.446-11])

# Guarda Volume Staff
team = Team.find_or_create_by!(name: "Equipe Guarda Volume Staff", sector: find_sector("Guarda Volume Staff", event))
add_members(team, %w[154.195.356-88])

# Lojinha
team = Team.find_or_create_by!(name: "Equipe Lojinha", sector: find_sector("Lojinha", event))
add_members(team, %w[162.735.106-01 087.004.756-61 113.976.276-11 704.232.906-65 123.242.986-43])

# Controle de Consumo
team = Team.find_or_create_by!(name: "Equipe Controle de Consumo", sector: find_sector("Controle de Consumo", event))
add_members(team, %w[023.157.276-00 133.441.326-60])

# Mega Fone
team = Team.find_or_create_by!(name: "Equipe Mega Fone", sector: find_sector("Mega Fone", event))
add_members(team, %w[141.085.536-83])

# Backstage
team = Team.find_or_create_by!(name: "Equipe Backstage", sector: find_sector("Backstage", event))
add_members(team, %w[125.432.956-07 143.799.776-76])

# Estoque Lojinha
team = Team.find_or_create_by!(name: "Equipe Estoque Lojinha", sector: find_sector("Estoque Lojinha", event))
add_members(team, %w[117.485.696-35])

# Caixa Ticket Social
team = Team.find_or_create_by!(name: "Equipe Caixa Ticket Social", sector: find_sector("Caixa Ticket Social", event))
add_members(team, %w[108.283.596-01 161.102.466-89 116.589.546-37 702.407.736-08 702.325.956-23 118.177.326-17])

# Troca de Alimento
team = Team.find_or_create_by!(name: "Equipe Troca de Alimento", sector: find_sector("Troca de Alimento", event))
add_members(team, %w[128.356.386-01 018.311.916-95 022.580.686-06 098.460.196-10 144.853.166-70])

# Bilheteria
team = Team.find_or_create_by!(name: "Equipe Bilheteria", sector: find_sector("Bilheteria", event))
add_members(team, %w[086.952.436-48 127.307.146-85 023.072.676-38 130.613.906-69])

# Caixas (sem setor específico, usar Bilheteria como setor pai)
team = Team.find_or_create_by!(name: "Equipe Caixas", sector: find_sector("Bilheteria", event))
